...
const emptyItem: LineItem = { description: "", hours: "", rate: "", expenses: "" };
...
export function InvoiceGenerator() {
  ...
  function expensesTotal(item: LineItem) {
    return parseFloat(item.expenses) || 0;
  }

  const subtotal = items.reduce((sum, item) => sum + laborTotal(item) + expensesTotal(item), 0);
  ...
  if (showPreview) {
    return (
      <div>
        <div id="invoice-preview" className="rounded-xl border border-slate-200 bg-white overflow-hidden print:border-0 print:rounded-none">
          <div className="bg-slate-900 px-6 py-4 text-white print:bg-slate-900 print:text-white">
            <p className="font-bold text-lg">{businessName || "Your Business Name"}</p>
            <p className="text-sm text-slate-400">Invoice · {new Date().toLocaleDateString()}</p>
          </div>
          <div className="px-6 py-5">
            ...
            <table className="w-full mt-4 text-sm">
              <thead>
                <tr className="border-b border-slate-200 text-slate-500">
                  <th className="text-left pb-2">Service / Deliverable</th>
              ...
              <tbody>
                {items.map((item, i) => (
                  <tr key={i} className="border-b border-slate-100">
                    <td className="py-2.5">{item.description || "Service"}</td>
                    <td className="py-2.5 text-right">{fmt(laborTotal(item))}</td>
                    <td className="py-2.5 text-right">{fmt(expensesTotal(item))}</td>
                    <td className="py-2.5 text-right font-medium">{fmt(laborTotal(item) + expensesTotal(item))}</td>
                  </tr>
                ))}
            ...
            <p className="mt-4 text-xs text-slate-400">Payment terms: Net 30</p>
          </div>
        </div>
      ...
      <div className="space-y-3">
        ...
        {items.map((item, i) => (
          <div key={i} className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-2 items-end">
            <div>
              {i === 0 && <span className="text-xs text-slate-500">Service / deliverable</span>}
              <input
                type="text"
            ...
            <div className="w-20">
              {i === 0 && <span className="text-xs text-slate-500">Hours</span>}
              <input
                type="number"
                value={item.hours}
            ...
            <div className="w-20">
              {i === 0 && <span className="text-xs text-slate-500">$/hr</span>}
              <input
                type="number"
                value={item.rate}
            ...
            <div className="w-24">
              {i === 0 && <span className="text-xs text-slate-500">Expenses $</span>}
              <input
                type="number"
                value={item.expenses}
                onChange={(e) => updateItem(i, "expenses", e.target.value)}
                placeholder="0"
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-slate-500 focus:outline-none"
              />
            </div>
