...
const emptyItem: LineItem = { description: "", hours: "", rate: "", materials: "" };
      ...
      <div>
        <div id="invoice-preview" className="rounded-xl border border-slate-200 bg-white overflow-hidden print:border-0 print:rounded-none">
          <div className="bg-slate-900 px-6 py-4 text-white print:bg-slate-900 print:text-white">
            <p className="font-bold text-lg">{businessName || "Your Construction Company"}</p>
          ...
          <div className="px-6 py-5">
              ...
              <tbody>
                {items.map((item, i) => (
                  <tr key={i} className="border-b border-slate-100">
                    <td className="py-2.5">{item.description || "Work item"}</td>
            ...
            <p className="mt-4 text-xs text-slate-400">Payment terms: Net 30 · License #XXXXXX</p>
          </div>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Kitchen renovation — 123 Oak St"
          className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-slate-500 focus:outline-none"
        />
      </div>
      ...
      <div className="space-y-3">
        ...
        {items.map((item, i) => (
          <div key={i} className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-2 items-end">
            <div>
              {i === 0 && <span className="text-xs text-slate-500">Work description</span>}
              <input
                type="text"
                value={item.description}
                onChange={(e) => updateItem(i, "description", e.target.value)}
                placeholder="e.g. Framing — master bedroom addition"
